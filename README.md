...
### 0. Easy Ubuntu Setup (Recommended)

If you are on Ubuntu/Debian and want an interactive terminal UI setup:

```bash
curl -fsSL https://raw.githubusercontent.com/changecoin938/Tunnel/main/install.sh | sudo bash
```

This installs `paqet` and launches `paqet-ui`, where you can pick:

- **Outside server (server role)**: generates and shows a pairing key
- **Inside server (client role)**: asks for outside IP + pairing key, then starts SOCKS5
...
### Pre-decrypt Guard (Recommended)

`paqet` can prepend a small authenticated header to each packet and **drop packets that don't match _before_ KCP decrypt**. This is useful to cheaply ignore random junk / disruption traffic hitting the server port.

Configuration (both sides must match):

- `transport.kcp.guard` (default: true)
- `transport.kcp.guard_magic` (default: `"PQT1"`, must be exactly 4 bytes)
- `transport.kcp.guard_window` (default: 30 seconds)
...
To profile CPU/memory under real traffic, you can enable Go `pprof`:

- `debug.pprof: "127.0.0.1:6060"`
